...
void lv_example_xml_1(void)
{
    ...
    lui_xml_register_component_from_data("red_button", red_button_xml);
    lui_xml_register_component_from_data("card", card_xml);
    lui_xml_register_component_from_data("motor_card", motor_card_xml);

    lv_obj_t * card;
    /*Create a card with the default values*/
    card = (lv_obj_t *) lui_xml_create(lv_screen_active(), "card", NULL);

    /*Create a motor card too. The returned value can be adjusted freely*/
    card = (lv_obj_t *) lui_xml_create(lv_screen_active(), "motor_card", NULL);
    ...
    card = (lv_obj_t *) lui_xml_create(lv_screen_active(), "card", attrs);
