...
lv_result_t lui_xml_register_translation_from_file(const char * path)
{
    lv_fs_res_t fs_res;
    lv_fs_file_t f;
    ...
    lv_result_t res = lui_xml_register_translation_from_data(xml_buf);
...
lv_result_t lui_xml_register_translation_from_data(const char * xml_def)
{
...
static void start_handler(void * user_data, const char * name, const char ** attrs)
{
    ...
    if(lv_streq(name, "translations")) {
        const char * languages = lui_xml_get_value_of(attrs, "languages");
        if(languages == NULL) {
        ...
        while(bufp[0]) {
            const char * lang = lui_xml_split_str(&bufp, ' ');
        ...
        const char * tag_name = lui_xml_get_value_of(attrs, "tag");
        if(tag_name == NULL) {
        ...
        for(i = 0; i < pack->language_cnt; i++) {
            const char * trans = lui_xml_get_value_of(attrs, pack->languages[i]);
            if(trans == NULL) {
