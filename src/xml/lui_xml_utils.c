...
const char * lui_xml_get_value_of(const char ** attrs, const char * name)
{
...
lv_color_t lui_xml_to_color(const char * str)
{
    /*fff, #fff, 0xfff*/
    if(lv_strlen(str) <= 5) return lv_color_hex3(lui_xml_strtol(str, NULL, 16));
    /*ffffff, #ffffff, 0xffffff*/
    else return lv_color_hex(lui_xml_strtol(str, NULL, 16));
}

lv_opa_t lui_xml_to_opa(const char * str)
{
    int32_t v = lui_xml_atoi(str);
...
bool lui_xml_to_bool(const char * str)
{
    return lv_streq(str, "false") ? false : true;
}

int32_t lui_xml_atoi_split(const char ** str, char delimiter)
{
    const char * s = *str;
    int32_t result = 0;
    int sign = 1;
...
int32_t lui_xml_atoi(const char * str)
{
    return lui_xml_atoi_split(&str, '\0');
}

#if LV_USE_FLOAT
float lui_xml_atof_split(const char ** str, char delimiter)
{
    const char * s = *str;
    float result = 0.0f;
    int sign = 1;
...
float lui_xml_atof(const char * str)
{
    return lui_xml_atof_split(&str, '\0');
}
#endif

int32_t lui_xml_strtol(const char * str, char ** endptr, int32_t base)
{
    const char * s = str;
    int32_t result = 0;
    int32_t sign = 1;
...
char * lui_xml_split_str(char ** src, char delimiter)
{
