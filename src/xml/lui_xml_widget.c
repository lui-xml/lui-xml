...
lv_widget_processor_t * lui_xml_widget_get_extended_widget_processor(const char * extends)
{
    ...
    while(extends) {
        proc = lui_xml_widget_get_processor(extends);
        if(proc) break;

        lui_xml_component_scope_t * extended_component = lui_xml_component_get_scope(extends);
    ...
    if(proc == NULL) {
        ...
        proc = lui_xml_widget_get_processor("lv_obj");
    }
