...
static void process_const_element(lui_xml_parser_state_t * state, const char ** attrs);
static void process_font_element(lui_xml_parser_state_t * state, const char * type, const char ** attrs);
static void process_image_element(lui_xml_parser_state_t * state, const char * type, const char ** attrs);
static void process_prop_element(lui_xml_parser_state_t * state, const char * name, const char ** attrs);
...
void lui_xml_component_init(void)
{
    lv_ll_init(&component_scope_ll, sizeof(lui_xml_component_scope_t));

    lui_xml_component_scope_t * global_scope = lv_ll_ins_head(&component_scope_ll);
    lv_memzero(global_scope, sizeof(lui_xml_component_scope_t));

    lui_xml_component_scope_init(global_scope);
    global_scope->name = lv_strdup("globals");
}

void lui_xml_component_scope_init(lui_xml_component_scope_t * scope)
{
    lv_ll_init(&scope->style_ll, sizeof(lui_xml_style_t));
    lv_ll_init(&scope->const_ll, sizeof(lui_xml_const_t));
    lv_ll_init(&scope->param_ll, sizeof(lui_xml_param_t));
    lv_ll_init(&scope->gradient_ll, sizeof(lui_xml_grad_t));
    lv_ll_init(&scope->subjects_ll, sizeof(lui_xml_subject_t));
    lv_ll_init(&scope->event_ll, sizeof(lui_xml_event_cb_t));
    lv_ll_init(&scope->image_ll, sizeof(lui_xml_image_t));
    lv_ll_init(&scope->font_ll, sizeof(lui_xml_font_t));
    lv_ll_init(&scope->timeline_ll, sizeof(lui_xml_timeline_t));
}


lv_obj_t * lui_xml_component_process(lui_xml_parser_state_t * state, const char * name, const char ** attrs)
{
    lui_xml_component_scope_t * scope = lui_xml_component_get_scope(name);
    if(scope == NULL) return NULL;
    lv_obj_t * item = lui_xml_create_in_scope(state->parent, &state->scope, scope, attrs);
    if(item == NULL) {
    ...
    state->item = item;
    lv_widget_processor_t * extended_proc = lui_xml_widget_get_extended_widget_processor(scope->extends);
    extended_proc->apply_cb(state, attrs);

#if LV_USE_OBJ_NAME
    /*Set a default indexed name*/
    if(state->item) {
        const char * value_of_name = lui_xml_get_value_of(attrs, "name");
...
lui_xml_component_scope_t * lui_xml_component_get_scope(const char * component_name)
{
...
lv_result_t lui_xml_register_component_from_data(const char * name, const char * xml_def)
{
    bool globals = false;
    ...
    if(globals) {
        lui_xml_component_scope_t * global_scope = lui_xml_component_get_scope("globals");
        lv_memcpy(global_scope, &state.scope, sizeof(lui_xml_component_scope_t));
    }
    else {
        lui_xml_component_scope_t * scope = lv_ll_ins_head(&component_scope_ll);
        lv_memzero(scope, sizeof(lui_xml_component_scope_t));
        lv_memcpy(scope, &state.scope, sizeof(lui_xml_component_scope_t));
        ...
        if(!scope->view_def) {
            ...
            lui_xml_unregister_component(name);
            return LV_RESULT_INVALID;
        }
    }
...
lv_result_t lui_xml_register_component_from_file(const char * path)
{
    ...
    lv_result_t res = lui_xml_register_component_from_data(filename, xml_buf);
...
lv_result_t lui_xml_unregister_component(const char * name)
{
    lui_xml_component_scope_t * scope = lui_xml_component_get_scope(name);
    ...
    lui_xml_timeline_t * timeline;
    LV_LL_READ(&scope->timeline_ll, timeline) {
        lui_xml_anim_timeline_child_t * child;
...
static void process_const_element(lui_xml_parser_state_t * state, const char ** attrs)
{
    const char * name = lui_xml_get_value_of(attrs, "name");
    const char * value = lui_xml_get_value_of(attrs, "value");
    ...
    lui_xml_register_const(&state->scope, name, value);
}
...
static void process_font_element(lui_xml_parser_state_t * state, const char * type, const char ** attrs)
{
    ...
    const char * src_path = lui_xml_get_value_of(attrs, "src_path");
    if(src_path == NULL) {
        LV_LOG_WARN("'src_path' is missing from a `%s` font", name);
        return;
    }

    const char * as_file = lui_xml_get_value_of(attrs, "as_file");
    if(as_file == NULL || lv_streq(as_file, "false")) {
        LV_LOG_INFO("Ignore non-file based font `%s`", name);
        return;
    }

    char src_path_full[LUI_XML_MAX_PATH_LENGTH];
    ...
    if(lv_streq(type, "tiny_ttf")) {
        const char * size = lui_xml_get_value_of(attrs, "size");
        if(size == NULL) {
            LV_LOG_WARN("'size' is missing from a `%s` tiny_ttf font", name);
            return;
        }
#if LV_TINY_TTF_FILE_SUPPORT
        lv_font_t * font = lv_tiny_ttf_create_file(src_path_full, lui_xml_atoi(size));
        if(font == NULL) {
            LV_LOG_WARN("Couldn't load  `%s` tiny_ttf font", name);
            return;
        }
        lv_result_t res = lui_xml_register_font(&state->scope, name, font);
...
static void process_image_element(lui_xml_parser_state_t * state, const char * type, const char ** attrs)
{
    ...
    if(lv_streq(type, "file")) {
        lui_xml_register_image(&state->scope, name, src_path);
    }
    else {
...
static void process_subject_element(lui_xml_parser_state_t * state, const char * type, const char ** attrs)
{
    const char * name = lui_xml_get_value_of(attrs, "name");
    const char * value = lui_xml_get_value_of(attrs, "value");
    ...
    const char * min_value_str = lui_xml_get_value_of(attrs, "min_value");
    const char * max_value_str = lui_xml_get_value_of(attrs, "max_value");

    if(lv_streq(type, "int")) {
        lv_subject_init_int(subject, lui_xml_atoi(value));
        if(min_value_str) lv_subject_set_min_value_int(subject, lui_xml_atoi(min_value_str));
        if(max_value_str) lv_subject_set_max_value_int(subject, lui_xml_atoi(max_value_str));
    }
#if LV_USE_FLOAT
    else if(lv_streq(type, "float")) {
        lv_subject_init_float(subject, lui_xml_atof(value));
        if(min_value_str) lv_subject_set_min_value_float(subject, lui_xml_atof(min_value_str));
        if(max_value_str) lv_subject_set_max_value_float(subject, lui_xml_atof(max_value_str));
    }
#endif
    else if(lv_streq(type, "color")) lv_subject_init_color(subject, lui_xml_to_color(value));
    ...
    lui_xml_register_subject(&state->scope, name, subject);
}

static void process_timeline_element(lui_xml_parser_state_t * state, const char ** attrs)
{
    const char * name = lui_xml_get_value_of(attrs, "name");
    ...
    lui_xml_register_timeline(&state->scope, name);

    /*Save the created timeline so that animations can be added to it later*/
    state->context = lui_xml_get_timeline(&state->scope, name);
}

static void process_animation_element(lui_xml_parser_state_t * state, const char ** attrs)
{
    ...
    const char * target_str = lui_xml_get_value_of(attrs, "target");
    const char * prop_str = lui_xml_get_value_of(attrs, "prop");
    const char * start_str = lui_xml_get_value_of(attrs, "start");
    const char * end_str = lui_xml_get_value_of(attrs, "end");
    const char * duration_str = lui_xml_get_value_of(attrs, "duration");
    const char * delay_str = lui_xml_get_value_of(attrs, "delay");
    const char * early_apply_str = lui_xml_get_value_of(attrs, "early_apply");
    const char * selector_str = lui_xml_get_value_of(attrs, "selector");
    ...
    lv_style_prop_t prop = lui_xml_style_prop_to_enum(prop_str);
    ...
    lv_style_selector_t selector = lui_xml_style_selector_text_to_enum(selector_str);

    if(target_str[0] == '#') target_str = lui_xml_get_const(&state->scope, &target_str[1]);
    if(prop_str[0] == '#') prop_str = lui_xml_get_const(&state->scope, &prop_str[1]);
    if(start_str[0] == '#') start_str = lui_xml_get_const(&state->scope, &start_str[1]);
    if(end_str[0] == '#') end_str = lui_xml_get_const(&state->scope, &end_str[1]);
    if(duration_str[0] == '#') duration_str = lui_xml_get_const(&state->scope, &duration_str[1]);
    if(delay_str[0] == '#') delay_str = lui_xml_get_const(&state->scope, &delay_str[1]);
    if(early_apply_str[0] == '#') early_apply_str = lui_xml_get_const(&state->scope, &early_apply_str[1]);

    lui_xml_timeline_t * at = state->context;
    if(at == NULL) {
    ...
    lui_xml_anim_timeline_child_t * child = lv_ll_ins_tail(&at->anims_ll);
    child->is_anim = true;
    ...
    if(prop_type == STYLE_PROP_TYPE_INT) {
        int32_t start = lui_xml_to_size(start_str);
        int32_t end = lui_xml_to_size(end_str);
        lv_anim_set_values(a, start, end);
        lv_anim_set_custom_exec_cb(a, anim_exec_cb);
    }
    else if(prop_type == STYLE_PROP_TYPE_OPA) {
        int32_t start = lui_xml_to_opa(start_str);
        int32_t end = lui_xml_to_opa(end_str);
        lv_anim_set_values(a, start, end);
        lv_anim_set_custom_exec_cb(a, anim_exec_cb);
    }
    else if(prop_type == STYLE_PROP_TYPE_COLOR) {
        anim_data->color_start = lui_xml_to_color(start_str);
        anim_data->color_end = lui_xml_to_color(end_str);
        lv_anim_set_values(a, 0, 255);
        lv_anim_set_custom_exec_cb(a, anim_exec_cb);
    }

    lv_anim_set_var(a, lv_strdup(target_str));
    lv_anim_set_duration(a, lui_xml_atoi(duration_str));
    lv_anim_set_delay(a, lui_xml_atoi(delay_str));
    lv_anim_set_early_apply(a, lui_xml_to_bool(early_apply_str));
    lv_anim_set_user_data(a, anim_data);
}

static void process_include_timeline_element(lui_xml_parser_state_t * state, const char ** attrs)
{
    lui_xml_timeline_t * at = state->context;
    if(at == NULL) {
        LV_LOG_INFO("No parent timeline is set, skipping");
    ...
    const char * target_str = lui_xml_get_value_of(attrs, "target");
    const char * timeline_str = lui_xml_get_value_of(attrs, "timeline");
    const char * delay_str = lui_xml_get_value_of(attrs, "delay");
    ...
    if(delay_str == NULL) delay_str = "0";
    if(target_str[0] == '#') target_str = lui_xml_get_const(&state->scope, &target_str[1]);
    if(timeline_str[0] == '#') timeline_str = lui_xml_get_const(&state->scope, &timeline_str[1]);
    if(delay_str[0] == '#') delay_str = lui_xml_get_const(&state->scope, &delay_str[1]);
    ...
    child->is_anim = false;
    child->data.incl.delay = lui_xml_atoi(delay_str);
...
static void process_grad_element(lui_xml_parser_state_t * state, const char * tag_name, const char ** attrs)
{
    lui_xml_grad_t * grad = lv_ll_ins_tail(&state->scope.gradient_ll);
    lv_memzero(grad, sizeof(lui_xml_grad_t));

    grad->name = lv_strdup(lui_xml_get_value_of(attrs, "name"));
    ...
    if(lv_streq(tag_name, "linear")) {
        ...
        char buf[64];
        char * buf_p = buf;
        const char * start = lui_xml_get_value_of(attrs, "start");
        lv_strlcpy(buf, start, sizeof(buf));
        dsc->params.linear.start.x = lui_xml_to_size(lui_xml_split_str(&buf_p, ' '));
        dsc->params.linear.start.y = lui_xml_to_size(buf_p);

        buf_p = buf;
        const char * end = lui_xml_get_value_of(attrs, "end");
        lv_strlcpy(buf, end, sizeof(buf));
        dsc->params.linear.end.x = lui_xml_to_size(lui_xml_split_str(&buf_p, ' '));
        dsc->params.linear.end.y = lui_xml_to_size(buf_p);
    }
    else if(lv_streq(tag_name, "radial")) {
        dsc->dir = LV_GRAD_DIR_RADIAL;
        char buf[64];
        char * buf_p = buf;
        const char * center = lui_xml_get_value_of(attrs, "center");
        if(center) {
            lv_strlcpy(buf, center, sizeof(buf));
            dsc->params.radial.end.x = lui_xml_to_size(lui_xml_split_str(&buf_p, ' '));
            dsc->params.radial.end.y = lui_xml_to_size(buf_p);
        }
        else {
        ...
        buf_p = buf;
        const char * center_edge = lui_xml_get_value_of(attrs, "edge");
        if(center_edge) {
            lv_strlcpy(buf, center_edge, sizeof(buf));
            dsc->params.radial.end_extent.x = lui_xml_to_size(lui_xml_split_str(&buf_p, ' '));
            dsc->params.radial.end_extent.y = lui_xml_to_size(buf_p);
        }
        else {
        ...
        buf_p = buf;
        const char * center_radius = lui_xml_get_value_of(attrs, "radius");
        if(center_radius) {
            int32_t r = lui_xml_atoi(center_radius);
            lv_strlcpy(buf, center_edge, sizeof(buf));
        ...
        buf_p = buf;
        const char * focal = lui_xml_get_value_of(attrs, "focal_center");
        if(focal) {
            lv_strlcpy(buf, focal, sizeof(buf));
            dsc->params.radial.focal.x = lui_xml_to_size(lui_xml_split_str(&buf_p, ' '));
            dsc->params.radial.focal.y = lui_xml_to_size(buf_p);
        }
        else {
        ...
        buf_p = buf;
        const char * focal_edge = lui_xml_get_value_of(attrs, "focal_edge");
        if(focal_edge) {
            lv_strlcpy(buf, focal_edge, sizeof(buf));
            dsc->params.radial.focal_extent.x = lui_xml_to_size(lui_xml_split_str(&buf_p, ' '));
            dsc->params.radial.focal_extent.y = lui_xml_to_size(buf_p);
        }
        else {
        ...
        buf_p = buf;
        const char * focal_radius = lui_xml_get_value_of(attrs, "focal_radius");
        if(focal_radius) {
            int32_t r = lui_xml_atoi(center_radius);
            lv_strlcpy(buf, center_edge, sizeof(buf));
        ...
        if(center) {
            lv_strlcpy(buf, center, sizeof(buf));
            dsc->params.conical.center.x = lui_xml_to_size(lui_xml_split_str(&buf_p, ' '));
            dsc->params.conical.center.y = lui_xml_to_size(buf_p);
        }
        else {
        ...
        buf_p = buf;
        const char * angle = lui_xml_get_value_of(attrs, "angle");
        if(angle) {
            lv_strlcpy(buf, angle, sizeof(buf));
            dsc->params.conical.start_angle = lui_xml_atoi(lui_xml_split_str(&buf_p, ' '));
            dsc->params.conical.end_angle = lui_xml_atoi(buf_p);
        }
        else {
...
static void process_grad_stop_element(lui_xml_parser_state_t * state, const char ** attrs)
{
    /*Add the stop to the last gradient*/
    lui_xml_grad_t * grad = lv_ll_get_tail(&state->scope.gradient_ll);
    lv_grad_dsc_t * dsc = &grad->grad_dsc;
    ...
    const char * color_value = lui_xml_get_value_of(attrs, "color");
    const char * opa_value = lui_xml_get_value_of(attrs, "opa");
    const char * offset_value = lui_xml_get_value_of(attrs, "offset");

    dsc->stops[idx].color = color_value ? lui_xml_to_color(color_value) : lv_color_black();
    dsc->stops[idx].opa = opa_value ? lui_xml_to_opa(opa_value) : LV_OPA_COVER;
    dsc->stops[idx].frac = offset_value ? lui_xml_to_opa(offset_value) : (uint8_t)((int32_t)idx * 255 /
...
static void process_prop_element(lui_xml_parser_state_t * state, const char * name, const char ** attrs)
{
    ...
    lui_xml_param_t * prop = lv_ll_ins_tail(&state->scope.param_ll);
    lv_memzero(prop, sizeof(lui_xml_param_t));

    prop->name = lv_strdup(lui_xml_get_value_of(attrs, "name"));
    const char * def = lui_xml_get_value_of(attrs, "default");
    if(def) prop->def = lv_strdup(def);
    else prop->def = NULL;

    const char * type = lui_xml_get_value_of(attrs, "type");
...
static void start_metadata_handler(void * user_data, const char * name, const char ** attrs)
{
    ...
    lui_xml_parser_section_t old_section = state->section;
    lui_xml_parser_start_section(state, name);
...
{
    lui_xml_parser_state_t * state = (lui_xml_parser_state_t *)user_data;
    lui_xml_parser_end_section(state, name);
}
