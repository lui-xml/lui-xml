...
lv_result_t lui_xml_update_from_data(const char * xml_def)
{
...
static void start_handler(void * user_data, const char * name, const char ** attrs)
{
    ...
    lv_widget_processor_t * proc = lui_xml_widget_get_processor(name);
    if(proc == NULL) {
        LV_LOG_WARN("%s is not a known widget", name);
        return;
    }

    const char * widget_name = lui_xml_get_value_of(attrs, "name");
    if(widget_name == NULL) {
