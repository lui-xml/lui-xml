...
#include "lui_xml_load_private.h"
#include "lui_xml_private.h"
#include "parsers/lui_xml_obj_parser.h"
#include "parsers/lui_xml_button_parser.h"
#include "parsers/lui_xml_label_parser.h"
#include "parsers/lui_xml_image_parser.h"
#include "parsers/lui_xml_bar_parser.h"
#include "parsers/lui_xml_slider_parser.h"
#include "parsers/lui_xml_tabview_parser.h"
#include "parsers/lui_xml_chart_parser.h"
#include "parsers/lui_xml_table_parser.h"
#include "parsers/lui_xml_dropdown_parser.h"
#include "parsers/lui_xml_imagebutton_parser.h"
#include "parsers/lui_xml_roller_parser.h"
#include "parsers/lui_xml_scale_parser.h"
#include "parsers/lui_xml_buttonmatrix_parser.h"
#include "parsers/lui_xml_spangroup_parser.h"
#include "parsers/lui_xml_textarea_parser.h"
#include "parsers/lui_xml_keyboard_parser.h"
#include "parsers/lui_xml_arc_parser.h"
#include "parsers/lui_xml_switch_parser.h"
#include "parsers/lui_xml_spinbox_parser.h"
#include "parsers/lui_xml_checkbox_parser.h"
#include "parsers/lui_xml_canvas_parser.h"
#include "parsers/lui_xml_calendar_parser.h"
#include "parsers/lui_xml_spinner_parser.h"
#include "parsers/lui_xml_qrcode_parser.h"
...
static void create_timeline_instances(lui_xml_parser_state_t * state);
...
void lui_xml_init(void)
{
    ...
    lui_xml_component_init();

    lui_xml_register_font(NULL, "lv_font_default", lv_font_get_default());

    lui_xml_register_widget("lv_obj", lui_xml_obj_create, lui_xml_obj_apply);

#if LV_USE_BUTTON
    lui_xml_register_widget("lv_button", lui_xml_button_create, lui_xml_button_apply);
#endif

#if LV_USE_LABEL
    lui_xml_register_widget("lv_label", lui_xml_label_create, lui_xml_label_apply);
#endif

#if LV_USE_IMAGE
    lui_xml_register_widget("lv_image", lui_xml_image_create, lui_xml_image_apply);
#endif

#if LV_USE_BAR
    lui_xml_register_widget("lv_bar", lui_xml_bar_create, lui_xml_bar_apply);
#endif

#if LV_USE_SLIDER
    lui_xml_register_widget("lv_slider", lui_xml_slider_create, lui_xml_slider_apply);
#endif

#if LV_USE_SPINBOX
    lui_xml_register_widget("lv_spinbox", lui_xml_spinbox_create, lui_xml_spinbox_apply);
#endif

#if LV_USE_TABVIEW
    lui_xml_register_widget("lv_tabview", lui_xml_tabview_create, lui_xml_tabview_apply);
    lui_xml_register_widget("lv_tabview-tab_bar", lui_xml_tabview_tab_bar_create, lui_xml_tabview_tab_bar_apply);
    lui_xml_register_widget("lv_tabview-tab", lui_xml_tabview_tab_create, lui_xml_tabview_tab_apply);
    lui_xml_register_widget("lv_tabview-tab_button", lui_xml_tabview_tab_button_create, lui_xml_tabview_tab_button_apply);
#endif

#if LV_USE_CHART
    lui_xml_register_widget("lv_chart", lui_xml_chart_create, lui_xml_chart_apply);
    lui_xml_register_widget("lv_chart-cursor", lui_xml_chart_cursor_create, lui_xml_chart_cursor_apply);
    lui_xml_register_widget("lv_chart-series", lui_xml_chart_series_create, lui_xml_chart_series_apply);
    lui_xml_register_widget("lv_chart-axis", lui_xml_chart_axis_create, lui_xml_chart_axis_apply);
#endif

#if LV_USE_TABLE
    lui_xml_register_widget("lv_table", lui_xml_table_create, lui_xml_table_apply);
    lui_xml_register_widget("lv_table-column", lui_xml_table_column_create, lui_xml_table_column_apply);
    lui_xml_register_widget("lv_table-cell", lui_xml_table_cell_create, lui_xml_table_cell_apply);
#endif

#if LV_USE_DROPDOWN
    lui_xml_register_widget("lv_dropdown", lui_xml_dropdown_create, lui_xml_dropdown_apply);
    lui_xml_register_widget("lv_dropdown-list", lui_xml_dropdown_list_create, lui_xml_dropdown_list_apply);
#endif

#if LV_USE_IMAGEBUTTON
    lui_xml_register_widget("lv_imagebutton", lui_xml_imagebutton_create, lui_xml_imagebutton_apply);
    lui_xml_register_widget("lv_imagebutton-src_left", lui_xml_imagebutton_src_left_create,
                           lui_xml_imagebutton_src_left_apply);
    lui_xml_register_widget("lv_imagebutton-src_right", lui_xml_imagebutton_src_right_create,
                           lui_xml_imagebutton_src_right_apply);
    lui_xml_register_widget("lv_imagebutton-src_mid", lui_xml_imagebutton_src_mid_create, lui_xml_imagebutton_src_mid_apply);
#endif

#if LV_USE_ROLLER
    lui_xml_register_widget("lv_roller", lui_xml_roller_create, lui_xml_roller_apply);
#endif

#if LV_USE_SCALE
    lui_xml_register_widget("lv_scale", lui_xml_scale_create, lui_xml_scale_apply);
    lui_xml_register_widget("lv_scale-section", lui_xml_scale_section_create, lui_xml_scale_section_apply);
#endif

#if LV_USE_SPAN
    lui_xml_register_widget("lv_spangroup", lui_xml_spangroup_create, lui_xml_spangroup_apply);
    lui_xml_register_widget("lv_spangroup-span", lui_xml_spangroup_span_create, lui_xml_spangroup_span_apply);
#endif

#if LV_USE_BUTTONMATRIX
    lui_xml_register_widget("lv_buttonmatrix", lui_xml_buttonmatrix_create, lui_xml_buttonmatrix_apply);
#endif

#if LV_USE_TEXTAREA
    lui_xml_register_widget("lv_textarea", lui_xml_textarea_create, lui_xml_textarea_apply);
#endif

#if LV_USE_KEYBOARD
    lui_xml_register_widget("lv_keyboard", lui_xml_keyboard_create, lui_xml_keyboard_apply);
#endif

#if LV_USE_ARC
    lui_xml_register_widget("lv_arc", lui_xml_arc_create, lui_xml_arc_apply);
#endif

#if LV_USE_SWITCH
    lui_xml_register_widget("lv_switch", lui_xml_switch_create, lui_xml_switch_apply);
#endif

#if LV_USE_CHECKBOX
    lui_xml_register_widget("lv_checkbox", lui_xml_checkbox_create, lui_xml_checkbox_apply);
#endif

#if LV_USE_CANVAS
    lui_xml_register_widget("lv_canvas", lui_xml_canvas_create, lui_xml_canvas_apply);
#endif

#if LV_USE_CALENDAR
    lui_xml_register_widget("lv_calendar", lui_xml_calendar_create, lui_xml_calendar_apply);
#if LV_USE_CALENDAR_HEADER_ARROW
    lui_xml_register_widget("lv_calendar-header_arrow", lui_xml_calendar_header_arrow_create,
                           lui_xml_calendar_header_arrow_apply);
#endif
#if LV_USE_CALENDAR_HEADER_DROPDOWN
    lui_xml_register_widget("lv_calendar-header_dropdown", lui_xml_calendar_header_dropdown_create,
                           lui_xml_calendar_header_dropdown_apply);
#endif
#endif

#if LV_USE_SPINNER
    lui_xml_register_widget("lv_spinner", lui_xml_spinner_create, lui_xml_spinner_apply);
#endif

#if LV_USE_QRCODE
    lui_xml_register_widget("lv_qrcode", lui_xml_qrcode_create, lui_xml_qrcode_apply);
#endif

    lui_xml_register_widget("lv_obj-style", lv_obj_xml_style_create, lv_obj_xml_style_apply);
    lui_xml_register_widget("lv_obj-remove_style", lv_obj_xml_remove_style_create, lv_obj_xml_remove_style_apply);
    lui_xml_register_widget("lv_obj-remove_style_all", lv_obj_xml_remove_style_all_create,
                           lv_obj_xml_remove_style_all_apply);

    lui_xml_register_widget("lv_obj-event_cb", lv_obj_xml_event_cb_create, lv_obj_xml_event_cb_apply);

    lui_xml_register_widget("lv_obj-subject_toggle_event", lv_obj_xml_subject_toggle_create,
                           lv_obj_xml_subject_toggle_apply);
    lui_xml_register_widget("lv_obj-subject_set_int_event", lv_obj_xml_subject_set_create, lv_obj_xml_subject_set_apply);
    lui_xml_register_widget("lv_obj-subject_set_float_event", lv_obj_xml_subject_set_create, lv_obj_xml_subject_set_apply);
    lui_xml_register_widget("lv_obj-subject_set_string_event", lv_obj_xml_subject_set_create, lv_obj_xml_subject_set_apply);
    lui_xml_register_widget("lv_obj-subject_increment_event", lv_obj_xml_subject_increment_create,
                           lv_obj_xml_subject_increment_apply);

    lui_xml_register_widget("lv_obj-screen_load_event", lv_obj_xml_screen_load_event_create,
                           lv_obj_xml_screen_load_event_apply);
    lui_xml_register_widget("lv_obj-screen_create_event", lv_obj_xml_screen_create_event_create,
                           lv_obj_xml_screen_create_event_apply);

    lui_xml_register_widget("lv_obj-play_timeline_event", lv_obj_xml_play_timeline_event_create,
                           lv_obj_xml_play_timeline_event_apply);

    lui_xml_register_widget("lv_obj-bind_style", lv_obj_xml_bind_style_create, lv_obj_xml_bind_style_apply);
    lui_xml_register_widget("lv_obj-bind_style_prop", lv_obj_xml_bind_style_prop_create, lv_obj_xml_bind_style_prop_apply);
    lui_xml_register_widget("lv_obj-bind_flag_if_eq", lv_obj_xml_bind_flag_create, lv_obj_xml_bind_flag_apply);
    lui_xml_register_widget("lv_obj-bind_flag_if_not_eq", lv_obj_xml_bind_flag_create, lv_obj_xml_bind_flag_apply);
    lui_xml_register_widget("lv_obj-bind_flag_if_gt", lv_obj_xml_bind_flag_create, lv_obj_xml_bind_flag_apply);
    lui_xml_register_widget("lv_obj-bind_flag_if_lt", lv_obj_xml_bind_flag_create, lv_obj_xml_bind_flag_apply);
    lui_xml_register_widget("lv_obj-bind_flag_if_ge", lv_obj_xml_bind_flag_create, lv_obj_xml_bind_flag_apply);
    lui_xml_register_widget("lv_obj-bind_flag_if_le", lv_obj_xml_bind_flag_create, lv_obj_xml_bind_flag_apply);

    lui_xml_register_widget("lv_obj-bind_state_if_eq", lv_obj_xml_bind_state_create, lv_obj_xml_bind_state_apply);
    lui_xml_register_widget("lv_obj-bind_state_if_not_eq", lv_obj_xml_bind_state_create, lv_obj_xml_bind_state_apply);
    lui_xml_register_widget("lv_obj-bind_state_if_gt", lv_obj_xml_bind_state_create, lv_obj_xml_bind_state_apply);
    lui_xml_register_widget("lv_obj-bind_state_if_lt", lv_obj_xml_bind_state_create, lv_obj_xml_bind_state_apply);
    lui_xml_register_widget("lv_obj-bind_state_if_ge", lv_obj_xml_bind_state_create, lv_obj_xml_bind_state_apply);
    lui_xml_register_widget("lv_obj-bind_state_if_le", lv_obj_xml_bind_state_create, lv_obj_xml_bind_state_apply);
...
void * lui_xml_create_in_scope(lv_obj_t * parent, lui_xml_component_scope_t * parent_scope,
                              lui_xml_component_scope_t * scope,
                              const char ** attrs)
{
    /* Initialize the parser state */
    lui_xml_parser_state_t state;
    lui_xml_parser_state_init(&state);
...
void * lui_xml_create(lv_obj_t * parent, const char * name, const char ** attrs)
{
    ...
    lv_widget_processor_t * p = lui_xml_widget_get_processor(name);
    if(p) {
        lui_xml_parser_state_t state;
        lui_xml_parser_state_init(&state);
        state.parent = parent;
    ...
    lui_xml_component_scope_t * scope = lui_xml_component_get_scope(name);
    if(scope) {
        item = lui_xml_create_in_scope(parent, NULL, scope, attrs);
        if(item == NULL) {
        ...
        if(attrs) {
            ...
            p = lui_xml_widget_get_extended_widget_processor(scope->extends);
            p->apply_cb(&state, attrs);
#if LV_USE_OBJ_NAME
            value_of_name = lui_xml_get_value_of(attrs, "name");
...
{
    return lui_xml_create(NULL, name, NULL);
...
lv_result_t lui_xml_register_timeline(lui_xml_component_scope_t * scope, const char * name)
{
    ...
    lv_ll_init(&at->anims_ll, sizeof(lui_xml_anim_timeline_child_t));
...
lv_result_t lui_xml_register_image(lui_xml_component_scope_t * scope, const char * name, const void * src)
{
    ...
    if(lv_image_src_get_type(src) == LV_IMAGE_SRC_FILE) {
        char buf[LUI_XML_MAX_PATH_LENGTH];
...
static const char * get_param_type(lui_xml_component_scope_t * scope, const char * name)
{
    lui_xml_param_t * p;
...
static const char * get_param_default(lui_xml_component_scope_t * scope, const char * name)
{
    lui_xml_param_t * p;
    LV_LL_READ(&scope->param_ll, p) {
...
static void resolve_params(lui_xml_component_scope_t * item_scope, lui_xml_component_scope_t * parent_scope,
                           const char ** item_attrs, const char ** parent_attrs)
{
    ...
    for(i = 0; item_attrs[i]; i += 2) {
        ...
        if(value[0] == '$') {
            ...
            const char * ext_value = lui_xml_get_value_of(parent_attrs, name_clean);
            if(ext_value) {
                ...
                if(ext_value[0] == '#' || ext_value[0] == '$') {
                    ...
                    lui_xml_style_t * s = lui_xml_get_style_by_name(parent_scope, ext_value);
...
static void resolve_consts(const char ** item_attrs, lui_xml_component_scope_t * scope)
{
    uint32_t i;
    for(i = 0; item_attrs[i]; i += 2) {
        ...
        if(value[0] == '#') {
            ...
            const char * const_value = lui_xml_get_const(scope, value_clean);
            if(const_value) {
...
static void view_start_element_handler(void * user_data, const char * name, const char ** attrs)
{
    lui_xml_parser_state_t * state = (lui_xml_parser_state_t *)user_data;
    state->tag_name = name;

    bool is_view = false;
    if(lv_streq(name, "view")) {
        const char * extends = lui_xml_get_value_of(attrs, "extends");
    ...
    if(state->item == NULL) {
        state->item = lui_xml_component_process(state, name, attrs);
    }
    ...
    if(state->item == NULL) {
        ...
        if(lv_strlen(name) >= sizeof(buf)) {
            ...
            char * bufp = buf;
            const char * comp_name = lui_xml_split_str(&bufp, '-');
            const char * slot_name = bufp;
            lui_xml_component_scope_t * comp_scope = lui_xml_component_get_scope(comp_name);
...
static void create_timeline_instances(lui_xml_parser_state_t * state)
{
    ...
    lui_xml_component_scope_t * scope = &state->scope;
    ...
    lui_xml_timeline_t * timeline_dsc;
    ...
    LV_LL_READ(&scope->timeline_ll, timeline_dsc) {
        ...
        lui_xml_anim_timeline_child_t * timeline_child;
        LV_LL_READ(&timeline_dsc->anims_ll, timeline_child) {
            ...
            /*Or include (merge) the referenced timelines*/
                ...
                lui_xml_anim_timeline_include_t * incl = &timeline_child->data.incl;
