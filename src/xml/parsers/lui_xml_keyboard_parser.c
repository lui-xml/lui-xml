/**
 * @file lui_xml_keyboard_parser.c
 *
 */
 ...
 *      INCLUDES
 *********************/
#include "lui_xml_keyboard_parser.h"
...
void * lui_xml_keyboard_create(lui_xml_parser_state_t * state, const char ** attrs)
{
    LV_UNUSED(attrs);
    void * item = lv_keyboard_create(lui_xml_state_get_parent(state));

    return item;
}

void lui_xml_keyboard_apply(lui_xml_parser_state_t * state, const char ** attrs)
{
    ...
    for(int i = 0; attrs[i]; i += 2) {
        ...
        if(lv_streq("popovers", name)) lv_keyboard_set_popovers(item, lui_xml_to_bool(value));
    }
}
