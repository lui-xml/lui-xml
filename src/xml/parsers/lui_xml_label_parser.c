 ...
 *      INCLUDES
 *********************/
#include "lui_xml_label_parser.h"
...
void * lui_xml_label_create(lui_xml_parser_state_t * state, const char ** attrs)
{
    LV_UNUSED(attrs);
    void * item = lv_label_create(lui_xml_state_get_parent(state));

    return item;
}

void lui_xml_label_apply(lui_xml_parser_state_t * state, const char ** attrs)
{
