...
static void apply_styles(lui_xml_parser_state_t * state, lv_obj_t * obj, const char * name, const char * value);
...
{
    LV_UNUSED(attrs);
    void * item = lv_obj_create(lui_xml_state_get_parent(state));
...
void lui_xml_obj_apply(lui_xml_parser_state_t * state, const char ** attrs)
{
    ...
    for(int i = 0; attrs[i]; i += 2) {
...
#endif
        if(lv_streq("x", name)) lv_obj_set_x(item, lui_xml_to_size(value));
        else if(lv_streq("y", name)) lv_obj_set_y(item, lui_xml_to_size(value));
        else if(lv_streq("width", name)) lv_obj_set_width(item, lui_xml_to_size(value));
        else if(lv_streq("height", name)) lv_obj_set_height(item, lui_xml_to_size(value));
        else if(lv_streq("align", name)) lv_obj_set_align(item, lui_xml_align_to_enum(value));
        else if(lv_streq("flex_flow", name)) lv_obj_set_flex_flow(item, lui_xml_flex_flow_to_enum(value));
        else if(lv_streq("flex_grow", name)) lv_obj_set_flex_grow(item, lui_xml_atoi(value));
        else if(lv_streq("ext_click_area", name)) lv_obj_set_ext_click_area(item, lui_xml_atoi(value));
        else if(lv_streq("scroll_snap_x", name)) lv_obj_set_scroll_snap_x(item, lui_xml_scroll_snap_to_enum(value));
        else if(lv_streq("scroll_snap_y", name)) lv_obj_set_scroll_snap_y(item, lui_xml_scroll_snap_to_enum(value));
        else if(lv_streq("scrollbar_mode", name)) lv_obj_set_scrollbar_mode(item, lui_xml_scrollbar_mode_to_enum(value));

        else if(lv_streq("hidden", name))               lv_obj_set_flag(item, LV_OBJ_FLAG_HIDDEN, lui_xml_to_bool(value));
        else if(lv_streq("clickable", name))            lv_obj_set_flag(item, LV_OBJ_FLAG_CLICKABLE, lui_xml_to_bool(value));
        else if(lv_streq("click_focusable", name))      lv_obj_set_flag(item, LV_OBJ_FLAG_CLICK_FOCUSABLE,
                                                                            lui_xml_to_bool(value));
        else if(lv_streq("checkable", name))            lv_obj_set_flag(item, LV_OBJ_FLAG_CHECKABLE, lui_xml_to_bool(value));
        else if(lv_streq("scrollable", name))           lv_obj_set_flag(item, LV_OBJ_FLAG_SCROLLABLE, lui_xml_to_bool(value));
                                                                            ...
                                                                            lui_xml_to_bool(value));
        else if(lv_streq("scroll_one", name))           lv_obj_set_flag(item, LV_OBJ_FLAG_SCROLL_ONE, lui_xml_to_bool(value));
                                                                            ...
                                                                            lui_xml_to_bool(value));
        else if(lv_streq("snappable", name))            lv_obj_set_flag(item, LV_OBJ_FLAG_SNAPPABLE, lui_xml_to_bool(value));
        else if(lv_streq("press_lock", name))           lv_obj_set_flag(item, LV_OBJ_FLAG_PRESS_LOCK, lui_xml_to_bool(value));
                                                                           ...
                                                                           lui_xml_to_bool(value));
                                                                            ...
                                                                            lui_xml_to_bool(value));
        else if(lv_streq("floating", name))             lv_obj_set_flag(item, LV_OBJ_FLAG_FLOATING, lui_xml_to_bool(value));
        ...
        else if(lv_streq("checked", name))  lv_obj_set_state(item, LV_STATE_CHECKED, lui_xml_to_bool(value));
        else if(lv_streq("focused", name))  lv_obj_set_state(item, LV_STATE_FOCUSED, lui_xml_to_bool(value));
        else if(lv_streq("focus_key", name)) lv_obj_set_state(item, LV_STATE_FOCUS_KEY, lui_xml_to_bool(value));
        else if(lv_streq("edited", name))   lv_obj_set_state(item, LV_STATE_EDITED, lui_xml_to_bool(value));
        else if(lv_streq("hovered", name))  lv_obj_set_state(item, LV_STATE_HOVERED, lui_xml_to_bool(value));
        else if(lv_streq("pressed", name))  lv_obj_set_state(item, LV_STATE_PRESSED, lui_xml_to_bool(value));
        else if(lv_streq("scrolled", name)) lv_obj_set_state(item, LV_STATE_SCROLLED, lui_xml_to_bool(value));
        else if(lv_streq("disabled", name)) lv_obj_set_state(item, LV_STATE_DISABLED, lui_xml_to_bool(value));
...
void lv_obj_xml_style_apply(lui_xml_parser_state_t * state, const char ** attrs)
{
    ...
    lui_xml_style_t * xml_style = lui_xml_get_style_by_name(&state->scope, name);
    if(xml_style == NULL) {
...
void lv_obj_xml_remove_style_apply(lui_xml_parser_state_t * state, const char ** attrs)
{
    ...
    const char * style_str = lui_xml_get_value_of(attrs, "name");
    const char * selector_str = lui_xml_get_value_of(attrs, "selector");

    lv_style_t * style = NULL;
    if(style_str) {
        lui_xml_style_t * xml_style = lui_xml_get_style_by_name(&state->scope, style_str);
        if(xml_style == NULL) {
...
{
    const char * trigger_str = lui_xml_get_value_of(attrs, "trigger");
    lv_event_code_t code = LV_EVENT_CLICKED;
    if(trigger_str) code = lui_xml_trigger_text_to_enum_value(trigger_str);
    if(code == LV_EVENT_LAST)  {
        LV_LOG_WARN("Couldn't add call function event because `%s` trigger is invalid.", trigger_str);
        return;
    }

    const char * cb_str = lui_xml_get_value_of(attrs, "callback");
    if(cb_str == NULL) {
        LV_LOG_WARN("callback is mandatory for event-call_function");
        return;
    }

    lv_obj_t * obj = lui_xml_state_get_parent(state);
    lv_event_cb_t cb = lui_xml_get_event_cb(&state->scope, cb_str);
    if(cb == NULL) {
        LV_LOG_WARN("Couldn't add call function event because `%s` callback is not found.", cb_str);
        return;
    }

    const char * user_data_str = lui_xml_get_value_of(attrs, "user_data");
    char * user_data = NULL;
...
{
    /*If the tag_name is */
    const char * subject_str =  lui_xml_get_value_of(attrs, "subject");
    const char * trigger_str =  lui_xml_get_value_of(attrs, "trigger");
    ...
    if(trigger_str) trigger = lui_xml_trigger_text_to_enum_value(trigger_str);
    if(trigger == LV_EVENT_LAST)  {
        LV_LOG_WARN("Couldn't apply <lv_obj-subject_toggle_event> because `%s` trigger is invalid.", trigger_str);
        return;
    }

    lv_subject_t * subject = lui_xml_get_subject(&state->scope, subject_str);
    if(subject == NULL) {
...
void lv_obj_xml_subject_set_apply(lui_xml_parser_state_t * state, const char ** attrs)
{
    ...
    const char * subject_str =  lui_xml_get_value_of(attrs, "subject");
    const char * trigger_str =  lui_xml_get_value_of(attrs, "trigger");
    const char * value_str =  lui_xml_get_value_of(attrs, "value");
    ...
    if(subject_type == LV_SUBJECT_TYPE_INT) {
        lv_obj_add_subject_set_int_event(item, subject, trigger, lui_xml_atoi(value_str));
    }
    else if(subject_type == LV_SUBJECT_TYPE_FLOAT) {
#if LV_USE_FLOAT
        lv_obj_add_subject_set_float_event(item, subject, trigger, lui_xml_atof(value_str));
#else
...
void lv_obj_xml_subject_increment_apply(lui_xml_parser_state_t * state, const char ** attrs)
{
    const char * subject_str =  lui_xml_get_value_of(attrs, "subject");
    const char * trigger_str =  lui_xml_get_value_of(attrs, "trigger");
    const char * step_str =  lui_xml_get_value_of(attrs, "step");
    ...
    const char * rollover_str =  lui_xml_get_value_of(attrs, "rollover");
    ...
    int32_t step = lui_xml_atoi(step_str);
    lv_subject_increment_dsc_t * dsc = lv_obj_add_subject_increment_event(item, subject, trigger, step);

    if(min_value_str) lv_obj_set_subject_increment_event_min_value(item, dsc, lui_xml_atoi(min_value_str));
    if(max_value_str) lv_obj_set_subject_increment_event_max_value(item, dsc, lui_xml_atoi(max_value_str));
    if(rollover_str) lv_obj_set_subject_increment_event_rollover(item, dsc, lui_xml_to_bool(rollover_str));
}
...
void lv_obj_xml_bind_style_apply(lui_xml_parser_state_t * state, const char ** attrs)
{
    ...
    const char * ref_value_str = lui_xml_get_value_of(attrs, "ref_value");
    if(ref_value_str == NULL) {
        LV_LOG_WARN("`ref_value` is missing in lv_obj bind_style");
        return;
    }

    int32_t ref_value = lui_xml_atoi(ref_value_str);
...
void lv_obj_xml_bind_flag_apply(lui_xml_parser_state_t * state, const char ** attrs)
{
    ...
    const char * subject_str =  lui_xml_get_value_of(attrs, "subject");
    const char * flag_str =  lui_xml_get_value_of(attrs, "flag");
    const char * ref_value_str = lui_xml_get_value_of(attrs, "ref_value");
...
void lv_obj_xml_bind_state_apply(lui_xml_parser_state_t * state, const char ** attrs)
{
    ...
    if(subject_str == NULL) {
        ...
        if(subject == NULL) {
            ...
            lv_state_t s = lui_xml_state_to_enum(state_str);
            int32_t ref_value = lui_xml_atoi(ref_value_str);
            void * item = lui_xml_state_get_item(state);
...
{
    const char * screen_str = lui_xml_get_value_of(attrs, "screen");
    const char * duration_str = lui_xml_get_value_of(attrs, "duration");
    const char * delay_str = lui_xml_get_value_of(attrs, "delay");
    const char * anim_type_str = lui_xml_get_value_of(attrs, "anim_type");
    const char * trigger_str = lui_xml_get_value_of(attrs, "trigger");
    ...
    lv_event_code_t trigger = lui_xml_trigger_text_to_enum_value(trigger_str);
    if(trigger == LV_EVENT_LAST)  {
        LV_LOG_WARN("Couldn't apply <screen_load_event> because `%s` trigger is invalid.", trigger_str);
        return;
    }

    int32_t duration = lui_xml_atoi(duration_str);
    int32_t delay = lui_xml_atoi(delay_str);
    lv_screen_load_anim_t anim_type = lui_xml_screen_load_anim_text_to_enum_value(anim_type_str);
...
void lv_obj_xml_play_timeline_event_apply(lui_xml_parser_state_t * state, const char ** attrs)
{
    ...
    const char * reverse_str = lui_xml_get_value_of(attrs, "reverse");
    ...
    dsc->delay = lui_xml_atoi(delay_str);
    dsc->reverse = lui_xml_to_bool(reverse_str);
...
static void apply_styles(lui_xml_parser_state_t * state, lv_obj_t * obj, const char * name, const char * value)
{
    ...
    const char * prop_name = lui_xml_style_string_process(name_local, &selector);

    SET_STYLE_IF(width, lui_xml_to_size(value));
    ...
    else SET_STYLE_IF(bg_grad, lui_xml_component_get_grad(&state->scope, value));

    else SET_STYLE_IF(bg_image_src, lui_xml_get_image(&state->scope, value));
    ...
    else SET_STYLE_IF(text_font, lui_xml_get_font(&state->scope, value));
    ...
    else SET_STYLE_IF(arc_image_src, lui_xml_get_image(&state->scope, value));
    ...
    else SET_STYLE_IF(bitmap_mask_src, lui_xml_get_image(&state->scope, value));
...
static void screen_create_on_trigger_event_cb(lv_event_t * e)
{
    ...
    lv_obj_t * screen = lui_xml_create(NULL, dsc->screen_name, NULL);
    if(screen == NULL) {
