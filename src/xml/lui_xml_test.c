...
static bool execute_step(lui_xml_test_step_t * step, uint32_t slowdown);
...
lv_result_t lui_xml_test_register_from_data(const char * xml_def, const char * ref_image_path_prefix)
{
    ...
    lv_result_t res = lui_xml_register_component_from_data(LV_TEST_NAME, xml_def);
    if(res != LV_RESULT_OK) {
...
lv_result_t lui_xml_test_register_from_file(const char * path, const char * ref_image_path_prefix)
{
    lv_fs_res_t fs_res;
    lv_fs_file_t f;
    ...
    /* Register the test */
    lv_result_t res = lui_xml_test_register_from_data(xml_buf, ref_image_path_prefix);
...
void lui_xml_unregister_test(void)
{
    uint32_t i;
    for(i = 0; i < test.step_cnt; i++) {
        lui_xml_test_step_type_t type = test.steps[i].type;
        if(type == LUI_XML_TEST_STEP_TYPE_CLICK_ON) {
            lv_free((void *)test.steps[i].param.str);
        }
        if(type == LUI_XML_TEST_STEP_TYPE_SET_LANGUAGE) {
            lv_free((void *)test.steps[i].param.str);
        }
        if(type == LUI_XML_TEST_STEP_TYPE_SCREENSHOT_COMPARE) {
            lv_free((void *)test.steps[i].param.screenshot_compare.path);
        }
        if(type == LUI_XML_TEST_STEP_TYPE_SUBJECT_SET) {
            lv_free((void *)test.steps[i].param.subject_set.value);
        }
        if(type == LUI_XML_TEST_STEP_TYPE_SUBJECT_COMPARE) {
    ...
    lui_xml_unregister_component(LV_TEST_NAME);
}

void lui_xml_test_run_init(void)
{
    ...
    lui_xml_component_scope_t * scope = lui_xml_component_get_scope(LV_TEST_NAME);
    lui_xml_component_scope_t * extends_scope = lui_xml_component_get_scope(scope->extends);
    lv_obj_t * act_screen = lv_screen_active();
    if(extends_scope && extends_scope->is_screen) {
        lv_obj_t * test_screen = lui_xml_create(NULL, LV_TEST_NAME, NULL);
        lv_screen_load(test_screen);
        lv_obj_delete(act_screen);
    }
    else {
        lv_obj_clean(act_screen);
        lui_xml_create(act_screen, LV_TEST_NAME, NULL);
    }
    lv_refr_now(normal_display);
    test.step_act = 0;
}

bool lui_xml_test_run_next(uint32_t slowdown)
{
...
bool lui_xml_test_run_to(uint32_t slowdown, uint32_t step)
{
    if(test.step_cnt == 0) {
    ...
    while(passed && test.step_act <= step) {
        passed = lui_xml_test_run_next(slowdown);
    }

    return passed;
}

void lui_xml_test_run_stop(void)
{
    if(cursor) {
...
uint32_t lui_xml_test_run_all(uint32_t slowdown)
{
    lui_xml_test_run_init();

    uint32_t failed_cnt = 0;
    uint32_t i;
    for(i = 0; i < test.step_cnt; i++) {
        bool passed = lui_xml_test_run_next(slowdown);
        if(!passed) failed_cnt++;
    }

    lui_xml_test_run_stop();

    return failed_cnt;
}


uint32_t lui_xml_test_get_step_count(void)
{
    return test.step_cnt;
}

uint32_t lui_xml_test_get_step_next(void)
{
    return test.step_act;
}

lui_xml_test_step_type_t lui_xml_test_get_step_type(uint32_t idx)
{
    if(idx >= test.step_cnt) return LUI_XML_TEST_STEP_TYPE_NONE;

    return test.steps[idx].type;

}

bool lui_xml_test_get_status(uint32_t idx)
{
    if(idx >= test.step_cnt) return LUI_XML_TEST_STEP_TYPE_NONE;
...
static void lui_xml_test_wait(uint32_t ms, uint32_t slowdown)
{
    lv_test_wait(ms);
...
static bool execute_step(lui_xml_test_step_t * step, uint32_t slowdown)
{
    ...
    if(step->type == LUI_XML_TEST_STEP_TYPE_CLICK_AT) {
        int32_t x = step->param.mouse_pos.x;
        int32_t y = step->param.mouse_pos.y;
        click_at(x, y, slowdown);
    }
    else if(step->type == LUI_XML_TEST_STEP_TYPE_CLICK_ON) {
    ...
    else if(step->type == LUI_XML_TEST_STEP_TYPE_PRESS) {
        lv_obj_add_state(cursor, LV_STATE_PRESSED);
        lv_test_mouse_press();
    }
    else if(step->type == LUI_XML_TEST_STEP_TYPE_RELEASE) {
        lv_obj_remove_state(cursor, LV_STATE_PRESSED);
        lv_test_mouse_release();
    }
    else if(step->type == LUI_XML_TEST_STEP_TYPE_MOVE_TO) {
        int32_t x = step->param.mouse_pos.x;
        int32_t y = step->param.mouse_pos.y;
        lv_test_mouse_move_to(x, y);
        lv_obj_set_pos(cursor, x, y);
    }
    else if(step->type == LUI_XML_TEST_STEP_TYPE_SCREENSHOT_COMPARE) {
    ...
    else if(step->type == LUI_XML_TEST_STEP_TYPE_WAIT) {
        lui_xml_test_wait(step->param.wait.ms, slowdown);
    }
    else if(step->type == LUI_XML_TEST_STEP_TYPE_FREEZE) {
        lv_delay_ms(step->param.freeze.ms * slowdown);
    }
    else if(step->type == LUI_XML_TEST_STEP_TYPE_SUBJECT_SET) {
        lv_subject_type_t type = step->param.subject_set.subject->type;
        if(type == LV_SUBJECT_TYPE_INT) {
            lv_subject_set_int(step->param.subject_set.subject, lui_xml_atoi(step->param.subject_set.value));
        }
    ...
    else if(step->type == LUI_XML_TEST_STEP_TYPE_SUBJECT_COMPARE) {
        ...
        if(type == LV_SUBJECT_TYPE_INT) {
            ...
            if(v != lui_xml_atoi(step->param.subject_compare.value)) {
    ...
    else if(step->type == LUI_XML_TEST_STEP_TYPE_SET_LANGUAGE) {
...
static void start_metadata_handler(void * user_data, const char * name, const char ** attrs)
{
    ...
    if(lv_streq(name, "click_at")) {
        test.step_cnt++;
        const char * x = lui_xml_get_value_of(attrs, "x");
        const char * y = lui_xml_get_value_of(attrs, "y");
        test.steps = lv_realloc(test.steps, sizeof(lui_xml_test_step_t) * test.step_cnt);
        uint32_t idx = test.step_cnt - 1;
        test.steps[idx].type = LUI_XML_TEST_STEP_TYPE_CLICK_AT;
        test.steps[idx].param.mouse_pos.x = lui_xml_atoi(x);
        test.steps[idx].param.mouse_pos.y = lui_xml_atoi(y);
    }
    else if(lv_streq(name, "click_on")) {
        const char * obj_name = lui_xml_get_value_of(attrs, "name");
        if(obj_name == NULL) {
        ...
        test.steps = lv_realloc(test.steps, sizeof(lui_xml_test_step_t) * test.step_cnt);
        uint32_t idx = test.step_cnt - 1;
        test.steps[idx].type = LUI_XML_TEST_STEP_TYPE_CLICK_ON;
        ...
        test.steps = lv_realloc(test.steps, sizeof(lui_xml_test_step_t) * test.step_cnt);
        uint32_t idx = test.step_cnt - 1;
        test.steps[idx].type = LUI_XML_TEST_STEP_TYPE_MOVE_TO;
        test.steps[idx].param.mouse_pos.x = lui_xml_atoi(x);
        test.steps[idx].param.mouse_pos.y = lui_xml_atoi(y);
    }
        ...
        test.steps = lv_realloc(test.steps, sizeof(lui_xml_test_step_t) * test.step_cnt);
        uint32_t idx = test.step_cnt - 1;
        test.steps[idx].type = LUI_XML_TEST_STEP_TYPE_PRESS;
    }
    else if(lv_streq(name, "release")) {
        test.step_cnt++;
        test.steps = lv_realloc(test.steps, sizeof(lui_xml_test_step_t) * test.step_cnt);
        uint32_t idx = test.step_cnt - 1;
        test.steps[idx].type = LUI_XML_TEST_STEP_TYPE_RELEASE;
    }
    else if(lv_streq(name, "screenshot_compare")) {
        test.step_cnt++;
        const char * path = lui_xml_get_value_of(attrs, "path");
        test.steps = lv_realloc(test.steps, sizeof(lui_xml_test_step_t) * test.step_cnt);
        uint32_t idx = test.step_cnt - 1;
        test.steps[idx].type = LUI_XML_TEST_STEP_TYPE_SCREENSHOT_COMPARE;
        ...
        test.step_cnt++;
        const char * ms = lui_xml_get_value_of(attrs, "ms");
        test.steps = lv_realloc(test.steps, sizeof(lui_xml_test_step_t) * test.step_cnt);
        uint32_t idx = test.step_cnt - 1;
        test.steps[idx].type = LUI_XML_TEST_STEP_TYPE_WAIT;
        test.steps[idx].param.wait.ms = lui_xml_atoi(ms);
    }
        ...
        test.steps = lv_realloc(test.steps, sizeof(lui_xml_test_step_t) * test.step_cnt);
        uint32_t idx = test.step_cnt - 1;
        test.steps[idx].type = LUI_XML_TEST_STEP_TYPE_FREEZE;
        test.steps[idx].param.freeze.ms = lui_xml_atoi(ms);
    }
        ...
        lv_subject_t * subject = lui_xml_get_subject(NULL, subject_str);
        if(subject == NULL) {
        ...
        test.steps = lv_realloc(test.steps, sizeof(lui_xml_test_step_t) * test.step_cnt);
        uint32_t idx = test.step_cnt - 1;
        test.steps[idx].type = LUI_XML_TEST_STEP_TYPE_SUBJECT_SET;
        ...
        test.steps = lv_realloc(test.steps, sizeof(lui_xml_test_step_t) * test.step_cnt);
        uint32_t idx = test.step_cnt - 1;
        test.steps[idx].type = LUI_XML_TEST_STEP_TYPE_SUBJECT_COMPARE;
        ...
        test.steps = lv_realloc(test.steps, sizeof(lui_xml_test_step_t) * test.step_cnt);
        uint32_t idx = test.step_cnt - 1;
        test.steps[idx].type = LUI_XML_TEST_STEP_TYPE_SET_LANGUAGE;
        test.steps[idx].param.str = lv_strdup(obj_name);
        LV_ASSERT_MALLOC(test.steps[idx].param.str);
    }
}
...
static void click_at(int32_t x, int32_t y, int32_t slowdown)
{
    lv_obj_remove_state(cursor, LV_STATE_PRESSED);
    lv_test_mouse_release();
    lui_xml_test_wait(50, slowdown);
    lv_test_mouse_move_to(x, y);
    lv_test_mouse_press();
    lv_obj_set_pos(cursor, x, y);
    lv_obj_add_state(cursor, LV_STATE_PRESSED);
    lui_xml_test_wait(100, slowdown);
    lv_test_mouse_release();
    lui_xml_test_wait(50, slowdown);
    lv_obj_remove_state(cursor, LV_STATE_PRESSED);
    lv_refr_now(NULL);
